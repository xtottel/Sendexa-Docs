...
portability.details.forEach(detail => {
  console.log(`${detail.number}: ${detail.from} → ${detail.to}`);
});
```
...
async function lookupWithDatabaseCache(phoneNumber) {
  ...
  const now = new Date();
  const cacheAge = cached ? (now - cached.updated_at) / 1000 / 60 / 60 / 24 : Infinity;
...
- ✅ **[Validate Numbers](/docs/lookup/validate)** - Verify phone numbers
- 📱 **[Line Type Detection](/docs/lookup/line-type)** - Detect line types
- 📧 **[Email Verification](/docs/lookup/email-verification)** - Validate emails
- 🛠️ **[API Reference](/docs/lookup/v1)** - Complete documentation