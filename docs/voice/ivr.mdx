...
```
┌──────────┐     ┌─────────────┐     ┌──────────────┐
│  Caller  │────▶│   Sendexa   │────▶│  Your Server │
│  Dials   │     │  (fetches   │     │  (returns    │
│  Number  │     │  VoiceXML)  │     │  VoiceXML)   │
└──────────┘     └─────────────┘     └──────────────┘
      │                  │                     │
      │◀─────────────────┘                     │
      │       Plays audio/speaks                │
...
function getRoutingDestination() {
  ...
  return {
    available: false,
    message:
      'Our office is closed. Our hours are 9 AM to 6 PM, Monday to Friday',
  };
}
  ...
  const xml = `
    <Response>
      <Gather numDigits="1" action="/set-language">
        ...
        <Say language="fr-FR">Appuyez sur 2 pour le français</Say>
...
- 📞 **[Make Calls](/docs/voice/make-call)** - Initiate IVR calls
- 📢 **[Broadcast](/docs/voice/broadcast)** - Send to multiple recipients
- 🗣️ **[Text-to-Speech](/docs/voice/tts)** - Generate speech
- 🔔 **[Webhooks](/docs/voice/webhooks)** - Handle call events
- 🛠️ **[API Reference](/docs/voice/v1)** - Complete documentation
