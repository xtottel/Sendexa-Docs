...
| `<phoneme>` | Phonetic pronunciation | `<phoneme ph="təˈmeɪtoʊ">tomato</phoneme>` |
...
function formatNumberForSpeech(number) {
  ...
  return number.toString().split('').map(d => numberWords[d]).join(' ');
}
...
// Good - Spoken text
const good = "Contact us by email at support at company dot com, or call us at two three three, five zero, one two three, four five six seven";
```
...
- 📞 **[Make Calls](/docs/voice/make-call)** - Use TTS in calls
- 📢 **[Broadcast](/docs/voice/broadcast)** - Broadcast with TTS
- 🎛️ **[Build IVR](/docs/voice/ivr)** - Interactive voice menus
