...
async function requestAccountDeletion(phone, userId) {
  await sendOTP({
    phone,
    from: 'YourApp',
    message: 'Confirm account deletion with code {code}. This action is permanent.',
...
1. 📝 **[Request OTP](/docs/verify/request)** - Send customizable OTPs
2. ✅ **[Verify OTP](/docs/verify/verify)** - Validate user codes
3. 🔄 **[Resend OTP](/docs/verify/resend)** - Handle resend requests
4. 🔔 **[Webhooks](/docs/verify/webhooks)** - Real-time notifications
5. 🛠️ **[API Reference](/docs/verify/v1)** - Complete documentation