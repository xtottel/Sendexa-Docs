...
| --------------------- | ---------- | --------------------------------------- |
| `POST /v1/sms/send`   | 60/min     | Single message                          |
| `POST /v1/sms/bulk`   | 10/min     | Bulk operations (up to 1000 recipients) |
| `GET /v1/sms/status`  | 100/min    | Status checks                           |
| `GET /v1/sms/history` | 30/min     | Message history                         |

### OTP Endpoints

| Endpoint              | Rate Limit | Notes                         |
| --------------------- | ---------- | ----------------------------- |
| `POST /v1/otp/send`   | 30/min     | OTP generation                |
| `POST /v1/otp/verify` | 100/min    | OTP verification              |
| `POST /v1/otp/resend` | 10/min     | OTP resend (per phone number) |

### Billing Endpoints

| Endpoint                  | Rate Limit | Notes               |
| ------------------------- | ---------- | ------------------- |
| `GET /v1/billing/balance` | 20/min     | Balance checks      |
| `POST /v1/billing/topup`  | 5/min      | Top-up requests     |
| `GET /v1/billing/history` | 30/min     | Transaction history |
...
async function sendWithBackoff(data, maxRetries = 5) {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      const retryAfter =
        error.response.data.details?.retry_after || Math.pow(2, attempt);
...
const rotator = new TokenRotator(['token1', 'token2', 'token3']);
...
for (let i = 0; i < 100; i++) {
  promises.push(
    sendSMS({ to: '+233500000000', message: `Test ${i}` }).catch((error) =>
