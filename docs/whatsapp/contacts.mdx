...
class ContactManager {
  ...
  async updateContact(contactId, updates) {
    const response = await axios.put(`${this.baseUrl}/${contactId}`, updates, {
      headers: {
        Authorization: `Bearer ${this.apiToken}`,
        'Content-Type': 'application/json',
      },
...
function validateContact(contact) {
  ...
  if (
    !contact.phone ||
    !/^\+?\d{10,15}$/.test(contact.phone.replace(/\s/g, ''))
  ) {
...
- 📢 **[Create Broadcasts](/docs/whatsapp/broadcasts)** - Message multiple contacts
- 📝 **[Message Templates](/docs/whatsapp/templates)** - Build reusable templates
- 🔔 **[Set Up Webhooks](/docs/whatsapp/webhooks)** - Real-time notifications
