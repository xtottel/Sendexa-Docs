  ...
  async resendOTP(destination, channel = 'sms', otpId = null) {
    ...
    if (timeSinceLastResend < this.resendCooldown) {
      const waitTime = Math.ceil(
        (this.resendCooldown - timeSinceLastResend) / 1000
      );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (data.errorType === 'RESEND_COOLDOWN') {
          throw new Error(
            `Wait ${data.data.retryAfter} seconds before resending`
          );
  ...
  return (
    <div>
      {countdown > 0 ? (
        ...
        <button onClick={() => handleResend(phone)}>Resend OTP</button>
      )}
    </div>
  );
}
...
async function resendWithFeedback(phone) {
  try {
    ...
    showMessage(
      `New OTP sent! Expires in ${result.data.expiry.amount} ${result.data.expiry.duration}`,
      'success'
    );
...
- ✅ **[Verify OTP](/docs/verify/verify)** - Validate user codes
- 📝 **[Request OTP](/docs/verify/request)** - Send initial OTP
- 🔔 **[Webhooks](/docs/verify/webhooks)** - Real-time notifications
- 📖 **[API Reference](/docs/verify/v1)** - Complete documentation
