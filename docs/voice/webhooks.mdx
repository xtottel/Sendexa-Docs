...
async function handleCallCompleted(event) {
  ...
  console.log(`Duration: ${event.duration}s, Cost: ${event.cost} ${event.currency}`);
...
async function handleCallFailed(event) {
  ...
  if (['BUSY', 'NO_ANSWER'].includes(event.error.code)) {
    ...
    if (retryCount < 3) {
      ...
      }, (retryCount + 1) * 60000); // Wait 1, 2, 3 minutes
    }
  }
}
```
...
- 📞 **[Make Calls](/docs/voice/make-call)** - Start making calls
- 📢 **[Broadcast](/docs/voice/broadcast)** - Send to multiple
