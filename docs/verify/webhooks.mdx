...
**Example URLs:**
```
✅ https://api.yourdomain.com/webhooks/otp
✅ https://yourdomain.com/api/v1/otp-events
❌ http://yourdomain.com/webhook (HTTP not allowed)
❌ http://localhost:3000/webhook (Not accessible)
```
...
async function handleOTPVerified(event) {
  ...
  await sendNotification(event.phone || event.email, 'Verification successful!');
}
...
- 📝 **[Request OTP](/docs/verify/request)** - Send OTP codes
- ✅ **[Verify OTP](/docs/verify/verify)** - Validate codes
- 🔄 **[Resend OTP](/docs/verify/resend)** - Handle resends
