...
```json
{
  "to": ["233501234567", "233509876543", "233543219876", "254712345678"]
}
```
...
// Send broadcast
const recipients = ['233501234567', '233509876543', '233543219876'];
...
async function sendCampaignMessage(voters, candidate) {
  ...
  for (const batch of batches) {
    await broadcastVoiceMessage(batch, candidate.phoneNumber, message);
...
async function announceSchoolClosure(parents, date, reason) {
  ...
  return await broadcastVoiceMessage(parents, schoolPhone, message);
}
...
async function sendLargeBroadcast(recipients, from, message) {
  ...
  console.log(
    `Sending to ${recipients.length} recipients in ${batches.length} batches`
  );
  ...
  for (let i = 0; i < batches.length; i++) {
    ...
    try {
      const result = await broadcastVoiceMessage(batches[i], from, message);
      results.push(result);
...
async function broadcastWithDNCCheck(recipients, message) {
  ...
  console.log(
    `Removed ${recipients.length - allowedRecipients.length} DNC numbers`
  );
...
async function broadcastWithRetry(recipients, message, maxAttempts = 3) {
  ...
  throw new Error(
    `Broadcast failed after ${maxAttempts} attempts: ${lastError.message}`
  );
}
```
...
await broadcastVoiceMessage(allRecipients, '233302123456', actualMessage);
```
...
- 🗣️ **[Text-to-Speech](/docs/voice/tts)** - Generate speech
- 📞 **[Make Calls](/docs/voice/make-call)** - Individual calls
- 🎛️ **[Build IVR](/docs/voice/ivr)** - Interactive menus
- 🔔 **[Webhooks](/docs/voice/webhooks)** - Real-time events
- 🛠️ **[API Reference](/docs/voice/v1)** - Complete docs
